	...
	<div class="main-content row">
		...
		<div class="main-content col col-4 col-sm-6">
				...
				<table class="form-table">
					...
					<tr>
						...
						<td class="nowrap">
							...
							<p  class="help"><?php echo esc_html__( 'Select "yes" if you want to synchronize your WordPress user fields with Mailchimp.', 'mailchimp-sync' ); ?></p>
						</td>
					</tr>
                    </tbody>
					...
					<tr valign="top">
						...
						<td>
							...
							<p class="help"><?php echo esc_html__( 'Select the Mailchimp audience to synchronize changes to.' ,'mailchimp-sync' ); ?></p>
						</td>
					</tr>
					...
					<tr valign="top">
						...
						<td>
							...
							<p class="help"><?php echo esc_html__( 'Select a specific user role to synchronize. Users without this role will be ignored.', 'mailchimp-sync' ); ?></p>
						</td>
					</tr>
					...
					<tr valign="top">
						...
						<td class="mc4wp-sync-field-map">
							...
							if( ! isset( $selected_list ) ) {
								echo '<p class="help">' , esc_html__( 'Please select a MailChimp list first (and then save your settings).', 'mailchimp-sync' ) , '</p>';
							} else {
								echo '<div>';
					...
					<tr id="skip-empty-user-fields">
						...
						<td class="nowrap">
							...
							<p class="help"><?php echo esc_html__( 'Select "Yes" to skip empty field values, ensuring data in Mailchimp is not cleared out.', 'mailchimp-sync' ); ?></p>
						</td>
					</tr>
                    ...
                    <tr>
                        ...
                        <td class="nowrap">
                            ...
                            <p class="help"><?php echo esc_html__( 'Select "yes" to synchronize changes from Mailchimp to WordPress too. This only works if your website is publicly accessible.', 'mailchimp-sync' ); ?></p>
                        </td>
                    </tr>
			...
			<?php if( $this->options['enabled'] && '' !== $this->options['list'] ) { ?>
                ...
                if( $number_of_pending_jobs > 0 ) {
                    echo '<p><a class="button" href="', add_query_arg( array( '_mc4wp_action' => 'process_user_sync_queue' ) ), '">', esc_html__( 'Process', 'mailchimp-sync' ), '</a></p>';
                }

                echo '<p class="help">', sprintf( wp_kses( __( 'Keep an eye on the <a href="%s">debug log</a> for any errors in the background sync process.', 'mailchimp-sync' ), array( 'a' => array( 'href' => array() ) ) ), admin_url( 'admin.php?page=mailchimp-for-wp-other' ) ), '</p>';
