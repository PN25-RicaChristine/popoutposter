...
class MC4WP_Logging_Admin
{
    ...
    public function enqueue_assets($suffix = '')
    {
        ...
        if ($page === 'mailchimp-for-wp-reports' && $tab === 'statistics') {
            ...
            // load flot
            wp_register_script('mc4wp-admin-statistics', $assets_url . '/js/admin-statistics' . $suffix .'.js', array(), $this->plugin->version(), true);
    ...
    public function show_statistics_page()
    {
        ...
        $graph->init();
        $settings = array( 'ticksize' => $graph->step_size );

        // add scripts
        wp_localize_script('mc4wp-admin-statistics', 'mc4wp_statistics_data', $graph->datasets);
        wp_localize_script('mc4wp-admin-statistics', 'mc4wp_statistics_settings', $settings);
