		...
		<table class="form-table">
            ...
            <tr valign="top">
                ...
                <td>
                    ...
                    <p class="help"><?php _e('This synchronizes all product data with Mailchimp.', 'mc4wp-ecommerce'); ?></p>
                </td>
            </tr>
            ...
            <tr valign="top" data-showif="<?php echo esc_attr(json_encode($config)); ?>">
				...
				<td>
					...
					<p class="help"><?php _e('This synchronizes all customer & order data with Mailchimp.', 'mc4wp-ecommerce'); ?></p>
				</td>
			</tr>
    ...
    _e('By default, only completed orders are sent to Mailchimp. Select "Yes" to send refunded, cancelled and pending orders too. This is only needed if you use the Order Notifications automation.', 'mc4wp-ecommerce'); ?>
			...
			<tr valign="top" data-showif="<?php echo esc_attr(json_encode($config)); ?>">
				...
				<td>
					...
					<p class="help"><?php printf(__('This allows you to <a href="%s">setup an abandoned cart recovery workflow in Mailchimp</a>.', 'mc4wp-ecommerce'), 'https://www.mc4wp.com/kb/enabling-abandoned-cart-recovery/#utm_source=wp-plugin&utm_medium=mc4wp-premium&utm_campaign=ecommerce-settings-page'); ?></p>
				</td>
			</tr>
...
if (! empty($settings['last_updated'])) {
    ...
    if ($queue) {
        ...
        echo sprintf('<pre class="margin" style="display: %s;">', isset($_GET['debug']) || isset($_GET['debug_queue']) ? '' : 'none');
        var_dump($queue->all());
        echo '</pre>';
