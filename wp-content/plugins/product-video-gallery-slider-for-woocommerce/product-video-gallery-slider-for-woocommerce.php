...
class wc_product_video_gallery
{
	...
	{       
    	...
    	<div class="col-50">
			...
			<p class="paid_color">03. Adaptive Height (Slider Height Based On Images Automatically).</p>
		...
		if (!is_admin()){
			if (class_exists( 'WooCommerce' ) && is_product()){
				...
				$translation_array = array(
					...
					'nickx_show_lightbox'=> get_option('nickx_show_lightbox'),'nickx_show_zoom'=> get_option('nickx_show_zoom'),'nickx_related'=> get_option('nickx_related'),
	...
	if (has_post_thumbnail()){
		...
		if($product_video_url!=''){
			if (strpos($product_video_url, 'youtube') > 0 || strpos($product_video_url, 'youtu') > 0) {
	   			...
	   			$htmlvideo = '<div><video width="100%" height="400px" id="product_video_iframe" video-type="html5" controls><source src="'.$product_video_url.'"><p>Your browser does not support HTML5</p></video><a href="'.$product_video_url.'?enablejsapi=1&wmode=opaque" class="nickx-popup fa fa-expand fancybox-media" data-fancybox="product-gallery"></a></div>';
		    }
		    else{
	   			$htmlvideo = '<div><iframe data-skip-lazy="" width="100%" height="400px" id="product_video_iframe" video-type="youtube" data_src="'.$product_video_url.'" src="" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>';
		    }
		}
	...
	if ($attachment_ids){
		...
		if($attachment_count>1 || !empty(get_post_meta(get_the_ID(),'_nickx_video_text_url',true))){
			...
			if(get_post_meta(get_the_ID(),'_nickx_video_text_url',true)!=''){
				$htmlthumb = apply_filters('woocommerce_single_product_image_thumbnail_html','<li title="video" id="video-thumbnail"><img id="product_video_img" data-skip-lazy="" width="150" height="150" src="'.wc_placeholder_img_src().'" class="attachment-thumbnail size-thumbnail" alt="" sizes="(max-width: 150px) 100vw, 150px"></li>','',$post->ID);
			}
			...
			foreach ($attachment_ids as $attachment_id){
				...
				echo apply_filters('woocommerce_single_product_image_thumbnail_html',sprintf('<li title="%s">%s</li>',esc_attr( $props['caption'] ),wp_get_attachment_image( $attachment_id, apply_filters( 'single_product_large_thumbnail_size', 'thumbnail' ), 0,array('data-skip-lazy' => 'true'))),$attachment_id,$post->ID);
			}
			echo ((get_option('nickx_place_of_the_video')!='yes' || !$extend->is_nickx_act_lic()) ? $htmlthumb : '').'</div>';
		}
	}
}
/*function product_gallery_callback($atts = array()){
    ob_start();
...
add_shortcode("product_gallery_shortcode","product_gallery_callback");*/