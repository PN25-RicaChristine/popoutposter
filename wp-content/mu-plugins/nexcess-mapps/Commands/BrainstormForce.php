...
class BrainstormForce {
	...
	*/
	...
	protected function get_brainstorm_api_url( $product_name = '' ) {
		...
		return ! empty( $product_name ) ? add_query_arg( array( 'referer' => 'activate-' . sanitize_title( $product_name ) ), $set_base_api_route ) : $set_base_api_route;
	}
	...
	protected function update_brainstorm_product_info( $active_product_slug, $api_response_args, $license_key = '' ) {
		...
		$set_existing_products = get_option( self::OPTION_NAME, array() );
	...
	public function activate( $args ) {
		...
		$maybe_valid_code = filter_var( $get_response_code, FILTER_VALIDATE_INT, array(
			'options' => array(
				'min_range' => 200,
				'max_range' => 299,
			),
