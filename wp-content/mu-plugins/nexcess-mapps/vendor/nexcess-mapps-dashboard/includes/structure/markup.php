...
/**
 * Handle displaying the screen when no data exists.
 *
 * @param  boolean $echo  Whether to echo or just return it.
 *
 * @return HTML
 */
function display_installer_missing_dataset( $echo = true ) {

	// Set an empty.
	$build  = '';

	// Start with a div.
	$build .= '<div class="wrap nexcess-mapps-dashboard-page-wrap">';

		// Display the headline if we have one.
		$build .= '<h1 class="nexcess-mapps-dashboard-intro-headline">' . get_admin_page_title() . '</h1>';

		// Display the text.
		$build .= '<p class="nexcess-mapps-dashboard-intro-subtitle">' . esc_html__( 'There was an error retrieving available plugin data. Please try again later.', 'nexcess-mapps-dashboard' ) . ' ¯\_(ツ)_/¯</p>';
