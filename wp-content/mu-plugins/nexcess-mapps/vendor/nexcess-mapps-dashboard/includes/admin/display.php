...
function setup_admin_menu_page() {
	...
	// Set up the args and load the actual page.
	$menu_setup = add_dashboard_page(
	...
	add_action( 'load-' . $menu_setup, __NAMESPACE__ . '\build_admin_info_tab' );

	// Add our hook for loading up tabs.
	do_action( Core\HOOK_PREFIX . 'dashboard_menu_loaded', $menu_setup );
}
...
function build_admin_display() {
	...
	if ( empty( $plugin_dataset ) || is_wp_error( $plugin_dataset ) ) {
		...
		// Show our "whoops" screen.
		Markup\display_installer_missing_dataset();
