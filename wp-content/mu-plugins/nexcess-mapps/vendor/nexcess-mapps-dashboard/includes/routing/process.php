...
function install_single_plugin_via_repo( $plugin_slug = '', $run_cli = false ) {
	...
	if ( false !== $run_cli ) {
		return attempt_single_plugin_install_via_shell( $plugin_slug, false );
	}
...
function install_single_plugin_via_source( $plugin_source = '', $run_cli = false ) {
	...
	// Pull out the path prefix for the http.
	$check_path_prefix  = substr( $plugin_source, 4 );

	// Now check if the file is local or not.
	$maybe_local_file   = 'http' !== $check_path_prefix ? false : true;

	// Run our shell install if we are allowed.
	if ( false !== $run_cli ) {
		return attempt_single_plugin_install_via_shell( $plugin_source, $maybe_local_file );
	}
 ...
 * @param  boolean $remote_file    Whether or not the install is a remote file (used for sanitizing).
 *
 * @return boolean
 */
function attempt_single_plugin_install_via_shell( $plugin_source = '', $remote_file = false ) {
