...
class AdminBar {
	...
	protected function getRootMenu() {
		...
		/**
		 * Override the menu title for the admin bar title.
		 *
		 * @param string $company_name The company name used as the menu title.
		 */
		$admin_bar_title = apply_filters( 'nexcess_mapps_branding_admin_bar_name', $company_name );

		return [
			'id'    => self::MENU_ID,
			'title' => $icon . $admin_bar_title,
		];
	}
}
