...
function boxshop_template_loop_short_description_listview(){
	...
	?>
		<div class="short-description list bbb" style="display: none" >
			<?php //boxshop_the_excerpt_max_words($list_limit_words, '', true, '', true); 
			//echo $post->ID;
			echo boxshop_get_the_excerpt_by_id($post->ID);
			?>
		</div>
	<?php
	endif;
...
function boxshop_template_single_print_email_buttons(){
	?>
	<div class="email">
		<a href="mailto:?subject=<?php echo esc_attr(sanitize_title(get_the_title())); ?>&amp;body=<?php echo esc_url(get_permalink()); ?>" title="email">
	...
	if( $yith_woocompare->is_frontend() || $is_ajax ) {
		if( get_option('yith_woocompare_compare_button_in_products_list') == 'yes' ){
			...
			function boxshop_add_compare_button_to_product_list(){
				...
				echo '<a class="compare" href="'.$yith_woocompare->obj->add_product_url( $product->get_id() ).'" data-product_id="'.$product->get_id().'" title="Compare">'.get_option('yith_woocompare_button_text').'</a>';
				echo '</div>';
			}
